...
class SettingListAdapter : RecyclerView.Adapter<SettingListAdapter.ViewHolder>() {
    ...
    private val settingItems = listOf(
        ...
        SettingItem("표시기IP", 			"순번표시기(클라이언트)의 IP를 입력해주세요",	 Const.Key.DisplayerSetting.DISPLAYER_IP),
        SettingItem("사운드파일위치", 	"사운드파일 위치를 입력해주세요(상대경로)",	 Const.Key.DisplayerSetting.PATH_SOUND_FILE),
        SettingItem("홍보영상파일위치",	"홍보영상파일 위치를 입력해주세요(상대경로)",	 Const.Key.DisplayerSetting.PATH_VIDEO_FILE),
        SettingItem("이미지파일위치", 	"이미지파일 위치를 입력해주세요(상대경로)",	 Const.Key.DisplayerSetting.PATH_IMG_FILE),
        SettingItem("순번발행기 환경설정", "",										 ""),
        SettingItem("발행기IP", 			"순번발행기(서버)의 IP를 입력해주세요",		 Const.Key.DisplayerSetting.IQS_IP),
        SettingItem("발행기PORT", 		"순번발행기(서버)의 PORT를 입력해주세요",		 Const.Key.DisplayerSetting.IQS_PORT),
        SettingItem("백업서버IP", 		"백업서버IP를 입력해주세요",					 Const.Key.DisplayerSetting.BK_SERVER_IP),
        SettingItem("백업서버PORT",	 	"백업서버의 PORT를 입력해주세요",			 Const.Key.DisplayerSetting.BK_SERVER_PORT),
        SettingItem("호출화면", 			"기본호출화면:0 확대호출화면:1 보조표시기화면:2",Const.Key.DisplayerSetting.CALL_VIEW)
